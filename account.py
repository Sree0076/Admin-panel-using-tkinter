...
def signup():
    if firstName_entry.get() == "" or lastName_entry.get() =="" or passwordName_entry.get()== "" or emailName_entry.\
        get() =="" or confirm_passwordName_entry.get() =="":
            messagebox.showerror("ERROR","FIll UP ALL FIELDS")
        ...
        try:
            ...
            cur.execute("INSERT INTO accounts VALUES(?,?,?,?)",(firstName_entry.get(), lastName_entry.get(), emailName_entry.get(), passwordName_entry.get()))
            connection.commit()
            connection.close()
            
            messagebox.showinfo('Succes', "SUCCESFULLY REGISTERED")
...
Login_headerText1.place(x=90, y=15)
...
def signin():
    ...
    finduser = 'SELECT * FROM accounts WHERE Email = ? and Password = ?'
    cursor.execute(finduser, [(Login_emailName_entry.get()),(Login_passwordName_entry.get())])
    ...
    if result:
        ...
        messagebox.showinfo("FAILED","USERNAME OR PASSWORD IS INCORRECT")
